...
class DCGM_AGENT:
    ...
    def init(self, manualOpMode=False, embeddedHostengine=True):
        ...
        if embeddedHostengine:
            ...
            self.dcgmHandle = pydcgm.DcgmHandle(opMode=opMode)
        
        else:
            print(("Connecting to a standalone hostengine with %s opmode..." % 
                ('manual' if manualOpMode else 'auto')))
            
            self.dcgmHandle = pydcgm.DcgmHandle(ipAddress='127.0.0.1', opMode=opMode)
        print("")
        ...
        self.dcgmGroup.AddGpu(supportedGPUs[0])
        ...
        if nvvs_installed():
            ...
            try:
                response = self.dcgmGroup.action.RunDiagnostic(dcgm_structs.DCGM_DIAG_LVL_SHORT)
        ...
        self.dcgmGroup.stats.WatchPidFields(1000000, 3600, 0)
        ...
        # try:
        #     pidInfo = self.dcgmGroup.stats.GetPidInfo(pid)
        ...
        #     try:
        #         response = self.dcgmGroup.action.RunDiagnostic(dcgm_structs.DCGM_DIAG_LVL_MED)
        #     except dcgmExceptionClass(dcgm_structs.DCGM_ST_NOT_CONFIGURED):
        #         print("One of the GPUs on your system is not supported by NVVS")
        #     except dcgmExceptionClass(dcgm_structs.DCGM_ST_NVVS_ERROR) as e:
        #         if not should_ignore_error(e):
...
if __name__ == "__main__":
    ...
    # PATH_TO_KERNELS = "/home/m8113/Desktop/phd_work/dev/cuda-samples/bin/x86_64/linux/release"
    # k = "vectorAdd"
    # k = "matrixMul"
    # w = 1024
    # t = 1024
    # cmd_args = f"bash -c \'echo $$; exec {PATH_TO_KERNELS}/{k} -nN={w} -nT={t}\'"
    # cmd_args = f"bash -c \'echo $$; exec {PATH_TO_KERNELS}/{k} -wA=2048 -hA=2048 -wB=2048 -hB=2048\'"

    # pid = os.popen(cmd_args).read().split('\n')[0]
    # os.sync()
    main.profile(141071)
